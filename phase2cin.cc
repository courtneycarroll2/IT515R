...
int main() {
	...
	initialize(grid);
	cout.read(reinterpret_cast<char*>(&iterations), sizeof(uint32_t));
	//sizeof tells how many bytes
	// reinterpret tells it to convert from pointer to uint32_t -> pointer to char
	cout.read(reinterpret_cast<char*>(&epsilon), sizeof(float));
	cout.read(reinterpret_cast<char*>(&rows), sizeof(uint32_t));
	cout.read(reinterpret_cast<char*>(&columns), sizeof(uint32_t));
	print_Grid(grid);
	return 0;
